...
│   ├── .run_scanner.lock       ← PID 锁文件（防重复运行，自动清理）
│   ├── scans/                  ← Phase 1-3 结果
│   └── daily/                  ← Phase 4 日报
│
└── logs/                       ← 📜 运行日志（按天分文件）
...
│ 🛡 防重复运行检测（杀残留进程）                       │
│ 1. 从 baostock API 拉取最新日线数据（45秒超时保护）   │
│ 2. 执行昨日待处理订单（今日开盘价成交）                │
│ 3. 生成明日信号（基于今日收盘数据）                    │
│ 4. 更新持仓/现金/待处理订单                           │
│ 5. WxPusher 推送日报到微信                           │
...
### Q11: baostock API 挂起了怎么办？

`query_history_k_data_plus()` 偶尔会无限挂起（网络瞬时故障导致），新版已添加以下保护：

- **45 秒超时**（`core/hs300_utils.py`）：每只股票的 baostock API 调用用 `signal.alarm` 加 45 秒超时，超时则跳过该股票，继续处理下一只
- **自动清理残留进程**（`run_scanner.py`）：启动时扫描 `/proc` 下所有进程，通过 cmdline 精确识别并杀掉卡死的 `run_scanner.py` 进程
- **PID 锁文件**（`output/.run_scanner.lock`）：退出时自动清理，防止重复运行冲突

如果遇到超时跳过某只股票的情况，日志中会输出 `[baostock] 获取 {code} 数据超时` 字样，不影响其他股票的处理。
